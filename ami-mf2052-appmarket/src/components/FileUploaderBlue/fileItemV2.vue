...
<template>
  <div class="fileItem">
    <div :style="selfStyleData.iconContainer"><FileIcon :style="selfStyleData.icon" :type="fileType" /></div>
...
export default {
  ...
  methods: {
    ...
    getFileMime() {
      const mime = this.file.file.type;
      const name = this.file.filename.split(".");
      const extension = name[name.length - 1];
      this.fileType = mime || mapping[extension] || mapping["txt"];
    },
  },
  mounted() {
