...
<template>
  ...
  >
    <template v-if="type === 'small'">
      <div style="padding: 24px 0 13px 0; text-align: center">
...
export default {
  ...
  methods: {
    ...
    msgWrongTypes(wrongTypes) {
      ...
      this.$message({
        message: this.config.wrongFileTypeAlert,
        type: "warning",
        duration: 2000,
      });
    },
    ...
    checkFileLen() {
      const len =
        this.config.mode === ComponentMode.edit ? this.viewLengthData + this.helper.fileList.length : this.helper.fileList.length;
    ...
    fileTypeHandle(list) {
      ...
      for (let index = 0; index < list.length; index++) {
        ...
        const t = element.name.split(".");
        const extension = t && t.length ? t[t.length - 1] : "";
        const type = element.type || getMimeByFileExtension(extension);
