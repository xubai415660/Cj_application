 ...
 * @Author: 袁徐澄
 * @Description: 数模下载
 * @Date: 2023-01-06 11:22:26
 * @LastEditTime: 2023-01-07 10:43:04
 * @FilePath: \ami-mf2052-appmarket\src\views\dataModel\modelDown.vue
-->
<template>
  <div class="detail">
    <div class="app-detail">
      <div :ApplicaData="ApplicaData">
        ...
        <div class="app-detail-introduce">
          <ami-model-title title="数模介绍" borderBottom />
          <p style="margin-top: 10px">{{ ApplicaData.dmIntro }}</p>
        </div>
        <!-- 算法  -->
        ...
        <div class="app-detail-Version">
          <ami-model-title title="数模版本" borderBottom>
            <!-- <span class="ami-span" @click="Go_version()">查看更多>></span> -->
          </ami-model-title>
          <div class="Version-eff">
            <span style="margin-right: 20px">V{{ ApplicaData.stdDmVerNo }}</span>
            <span>{{ ApplicaData.issueTime }}</span>
            <p>{{ ApplicaData.verComment }}</p>
          </div>
        </div>
        <!-- 评论及评分 -->
        <div class="app-detail-Comment">
            ...
            <div class="Comment-wrap">
              <div v-for="(score, index) in newdataList" :key="index">
                ...
                <div class="Comment-wrap-countNum">
                  <ami-progress :percentage="Number(score.countEvalScore)" :show-text="false" color="#44D7B7"></ami-progress>
                </div>
              </div>
            </div>
          </div>
          ...
          <ami-carousel
            ...
            height="210px"
          >
            <ami-carousel-item v-for="(commit, index) in ApplicaData.currentUserList" :key="index" class="carousel-row">
              <div class="carousel-mian">
                <ami-avatar
                  class="carousel-img"
                  src="https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png"
                ></ami-avatar>
                <span class="carousel-name">{{ commit.userName }}</span>
...
import { selCurrentVersionInfo, inModelReview } from "@/api/model/model.js";
import Appdetails from "@/components/Appdetails/model_head.vue";
export default {
  ...
  created() {
    ...
    this.Model = this.$route.query.appCls; 
  ...
  methods: {
    ...
    loadTable() {
      let params = {
        verId: this.$route.query.verId,
        // appVerId: this.$route.query.appVerId,
        // appCls: this.$route.query.appCls,
        // mgtOrgCode: this.userInfo.mgtOrgCode,
      };
      selCurrentVersionInfo(params).then((res) => {
        if (res.data.code === 200) {
          this.ApplicaData = res.data.data;
          // this.queryScore();
          this.scoreList = res.data.data.cuList;
          //   this.queryComment();
          //   const dataList = res.data.data;
          //   res.data.data.forEach((item) => {
          //     this.scoreList[0].evalScore = item.evalScoreNumFive;
          //     this.scoreList[0].countNum = item.evalScoreNumFive;
          //   });
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let params = {
            ...
            // evalDesc: "评价描述",
            // evalScore: "5",
            stdDmVerNo: this.ApplicaData.stdDmVerNo,
          };
          inModelReview(params).then((res) => {
            if (res.data.code === 200) {
              this.operate = false;
