      ...
      <div class="suggestion">
        ...
        <div class="contRinght">
          ...
          <div class="cont-start">
            <span @click="toDetail(item)">{{ item.appName }}</span>
            ...
            <span v-if="item.dlNum === null">0次下载</span>
            <span v-else>{{ item.dlNum }}次下载</span>
          </div>
          <h3 class="cont-center">
            <div class="cont-center-dz">{{ item.busClsName }}</div>
            <span>{{ item.createTime | getYearMonthDay }}</span>
            <!-- 类型 -->
            <span>{{ item.appManufacturer === "" ? "--" : item.appManufacturer }}</span>
          </h3>
          <!-- 应用简介 -->
          <ami-tooltip :content="item.appInfo" :disabled="item.isShow" placement="bottom" effect="light">
            <p class="cont-p">{{ item.appInfo }}</p>
          </ami-tooltip>
        </div>
      </div>
    </div>
