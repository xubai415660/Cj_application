  ...
  methods: {
    ...
    queryTableData(flag) {
      ...
      request({
        ...
        .then((res) => {
          this.loading = false;
          const data = this.dataCallback ? this.dataCallback(this.columns, res.data.data) : res.data.data;
    ...
    exportXlsx() {
      ...
      let params = {
        filter: { ...filter },
        pageIndex: this.exportData.pageIndex,
    ...
    setTableHeaderHeight() {
      const searchNode = (wrapper, key) => {
        let res = null;

        for (let node of wrapper) {
          if (node.className && node.className.indexOf(key) > -1) {
            res = node;
            break;
          }
        }
      ...
      setTimeout(() => {
        if (this.$refs.tableC.$el && this.$refs.tableC.$el.childNodes) {
          const header = searchNode(this.$refs.tableC.$el.childNodes, "ami-table__header-wrapper");
          const header_h = header.clientHeight;
          this.table_header_h = header_h + "px";
        }
      }, 200);
    },
  },
...
.tables /deep/ .ami-table__body-wrapper {
  ...
  height: calc(100% - var(--header_h)) !important;
}
